...
POST_UNPACK_COMMANDS = [
    ({'command': 'cp -rf ${app_root}/nuxeo/templates/default ${app_root}/nuxeo/templates/market'}),
    ({'command': "sed -i -e 's/templates=default/templates=market/g' ${app_root}/nuxeo/templates/nuxeo.defaults"}),
    ({'command': 'chown -R ${root_user}:${root_group} ${app_root}'}),
    ({'command': 'chown -R ${web_user}:${web_group} ${app_root}/nuxeo'}),
]
...
source!%(src_num)d!nick = Nuxeo-DAM %(src_num)d
...
URL_TARGET        = "/market/install/nuxeo-dam/config/target"
URL_SERVER_CONFIG = "/market/install/nuxeo-dam/config/server"
URL_ACCESS        = "/market/install/nuxeo-dam/access/user"
...
class Server_Config (Install_Stage):
    def __safe_call__ (self):
        ...
        props.update (locals())
        tools.create_configuration (root, src_port)
...
class Access_Details (Install_Stage):
    def __safe_call__ (self):
        ...
        box += CTK.RawHTML ('<p>%s</p>' %(_('Nuxeo-DAM has been installed, and Cherokee is configured to automatically launch it whenever the resource is accessed if it is not already available. A one-time pre-launch has been issued to save time.')))
        box += CTK.RawHTML ('<p>%s</p>' %(_('Note that launching Nuxeo-DAM can take up a couple of  minutes on modern servers, so please be patient the first time you access it.')))
