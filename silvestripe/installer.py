...
POST_UNPACK_COMMANDS = [
    ...
    ({'command': 'chown -R ${web_user}:${web_group} ${app_root}/silverstripe'}),
]

# Cfg chunks
CONFIG_VSERVER = """
%(pre_vsrv)s!nick = %(target_vserver)s
%(pre_vsrv)s!document_root = %(root)s/silverstripe
%(pre_vsrv)s!directory_index = index.php,index.html

%(pre_vsrv)s!error_handler = error_redir
%(pre_vsrv)s!error_handler!404!show = 0
%(pre_vsrv)s!error_handler!404!url = /sapphire/main.php

%(pre_rule_plus1)s!handler = redir
%(pre_rule_plus1)s!handler!rewrite!10!regex = .*
%(pre_rule_plus1)s!handler!rewrite!10!show = 0
%(pre_rule_plus1)s!handler!rewrite!10!substring = /sapphire/main.php
%(pre_rule_plus1)s!match = fullpath
%(pre_rule_plus1)s!match!fullpath!1 = /index.php

# The PHP rule comes here

%(pre_rule_minus1)s!handler = redir
%(pre_rule_minus1)s!handler!iocache = 1
%(pre_rule_minus1)s!handler!rewrite!10!regex = ^([^\?]*)\??(.*)
%(pre_rule_minus1)s!handler!rewrite!10!show = 0
%(pre_rule_minus1)s!handler!rewrite!10!substring = sapphire/main.php?url=$1&$2
%(pre_rule_minus1)s!match = not
...
%(pre_rule_minus1)s!match!right!match_only_files = 0
...
%(pre_rule_plus2)s!handler!rewrite!10!regex = .*
%(pre_rule_plus2)s!handler!rewrite!10!show = 0
%(pre_rule_plus2)s!handler!rewrite!10!substring = %(target_directory)s/sapphire/main.php
%(pre_rule_plus2)s!match = fullpath
%(pre_rule_plus2)s!match!fullpath!1 = %(target_directory)s/index.php

%(pre_rule_plus1)s!document_root = %(root)s/silverstripe
...
%(pre_rule_minus1)s!handler!rewrite!10!regex = ^([^\?]*)\??(.*)
%(pre_rule_minus1)s!handler!rewrite!10!show = 0
%(pre_rule_minus1)s!handler!rewrite!10!substring = %(target_directory)s/sapphire/main.php?url=$1&$2
%(pre_rule_minus1)s!match = and
%(pre_rule_minus1)s!match!right = directory
%(pre_rule_minus1)s!match!right!directory = %(target_directory)s
%(pre_rule_minus1)s!match!left = not
%(pre_rule_minus1)s!match!left!right = exists
%(pre_rule_minus1)s!match!left!right!iocache = 1
%(pre_rule_minus1)s!match!left!right!match_any = 1
%(pre_rule_minus1)s!match!left!right!match_index_files = 1
%(pre_rule_minus1)s!match!left!right!match_only_files = 0
...
URL_TARGET       = "/market/install/silverstripe/target"
URL_SERVER       = "/market/install/silverstripe/server"
...
class Precondition (Install_Stage):
    def __safe_call__ (self):
        ...
        if setting_errors:
            box += CTK.RawHTML('<p>%s</p>' %(_("Some of your PHP settings do not meet SilverStripe's requirements. Please edit your PHP configuration files and try again.")))
...
class Post_Install (Install_Stage):
    def __safe_call__ (self):
        ...
        commands = [({'command': 'chown -R ${php_user}:${php_group} ${app_root}/silverstripe'})]
