...
POST_UNPACK_COMMANDS = [
    ...
    ({'command': 'chown root:${web_group} ${app_root}/statusnet ${app_root}/statusnet/avatar ${app_root}/statusnet/background ${app_root}/statusnet/file'}),
    ({'command': 'chmod g+w ${app_root}/statusnet ${app_root}/statusnet/avatar ${app_root}/statusnet/background ${app_root}/statusnet/file'}),
]

# Cfg chunks
CONFIG_VSERVER = """
%(pre_vsrv)s!nick = %(target_vserver)s
%(pre_vsrv)s!document_root = %(root)s/statusnet
...
CONFIG_DIR = """
%(pre_rule_plus1)s!document_root = %(root)s/statusnet
...
URL_TARGET       = "/market/install/statusnet/target"
URL_DATABASE     = php.URL_PHP_CONFIG_DONE
URL_SERVER       = "/market/install/statusnet/server"
URL_POST_INSTALL = "/market/install/statusnet/post_installation"
...
class Post_Install (Install_Stage):
    def __safe_call__ (self):
        ...
        commands = [({'function': tools.php_installer_replacements,
                    ...
                    ({'command': 'chown ${root_user}:${php_group} ${app_root}/statusnet ${app_root}/statusnet/avatar ${app_root}/statusnet/background ${app_root}/statusnet/file'}),]
